...
def test_history_exposes_positive_source_witness_but_no_pinpoint():
    part = aqv.FootnotePart(
        verbatim="Example v Test, 2024 SCC 1 at para 42",
        corrected="Example v Test, 2024 SCC 1 at para 42",
        kind="case",
        link="https://example.test/case#par57",
        pinpoint_fragments=["par42"],
        authority_id="a2aj:authority",
        source_evidence_found=True,
    )

    history = aqv._build_footnote_history_entries([part])

    assert "authority_id" not in history
    assert "source_evidence_found: true" in history
    assert "https://example.test/case" in history
    assert "par42" not in history
    assert "par57" not in history


def test_interleaved_witness_does_not_encode_match_location():
    document = aqv.a2aj_client.A2AJDocument(
        dataset="SCC", citation="2024 SCC 1", alternate_citation="",
        name="Example v Test", date="", url="https://example.test/case",
        text="The complete quoted proposition appears here.", language="en",
        scraped_timestamp="", upstream_license="", raw={},
    )
    lookup = aqv.a2aj_client.A2AJLookup("found", document, "exact_citation")
    part = aqv.FootnotePart(
        verbatim="Example v Test, 2024 SCC 1 at para 42",
        corrected="Example v Test, 2024 SCC 1 at para 42",
        kind="case", link="https://example.test/case#par42",
        pinpoint_fragments=["par42"], bare_citation="2024 SCC 1",
    )
    with patch.object(aqv, "USE_A2AJ", True), \
         patch.object(aqv.a2aj_client, "lookup_document", return_value=lookup), \
         patch.object(aqv, "_normalized_a2aj_document_text", return_value=document.text):
        [enriched] = aqv._interleave_a2aj_evidence(
            [part], 'The court said “The complete quoted proposition appears here.”'
        )

    assert enriched.authority_id.startswith("a2aj:")
    assert enriched.source_evidence_found is True
    history = aqv._build_footnote_history_entries([enriched])
    assert "par42" not in history
    assert "appears here" not in history
